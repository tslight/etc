...
#+begin_src emacs-lisp
  ...
  (setq user-full-name "~Not Toby"
        user-mail-address "0x2b@0xff.art")

  (setq view-read-only t) ;; pager bindings in all RO buffers
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; these are far superior to the defaults imho
...
#+begin_src emacs-lisp
  ...
  (add-hook 'emacs-startup-hook #'my/run-after-startup)
#+end_src
...
#+begin_src emacs-lisp
  (defun my/reopen-as-root ()
    ...
    (interactive)
    (find-alternate-file
     (concat (if (executable-find "doas") "/doas::" "/sudo::") buffer-file-name)))

  (defun my/find-file-as-root (file-name)
    "Like find file, but opens the file as root."
    (interactive "FFind file (as root): ")
    (find-file
     (concat (if (executable-find "doas") "/doas::" "/sudo::") file-name)))

  (keymap-global-set "C-c f s" 'my/reopen-as-root)
  (keymap-global-set "C-x M-f" 'my/find-file-as-root)
#+end_src

** system shenanignans

This are used in tangle logic in other files

#+begin_src emacs-lisp
  (defun apple-p ()
    (and (eq 0 (shell-command "grep -iq apple /var/log/messages"))))

  (defun lenovo-p ()
    (and (eq 0 (shell-command "grep -iq lenovo /var/log/messages"))))
  
  (defun freebsd-p ()
    (and (string-match-p "freebsd" system-configuration)))

  (defun openbsd-p ()
    (and (string-match-p "openbsd" system-configuration)))

  (defun apple-freebsd-p () (and (apple-p) (freebsd-p)))
  (defun apple-openbsd-p () (and (apple-p) (openbsd-p)))
  (defun lenovo-freebsd-p () (and (lenovo-p) (freebsd-p)))
  (defun lenovo-openbsd-p () (and (lenovo-p) (openbsd-p)))

  (defun retina-display-p ()
    (> (string-to-number
        (shell-command-to-string
         "xdpyinfo | grep dimensions | awk '{print $2}' | awk -Fx '{print $1}'"))
       2000))
#+end_src
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/eshell/alias :comments no
  alias g git $@*
  alias l ls -a $@*
  alias ll ls -alh $@*
  alias pg pgrep -l $@*
...
#+begin_src emacs-lisp
  (use-package org
    ...
    ;; stop electric pair from closing > when spawning a src block
...
#+begin_src emacs-lisp
  (when (and (not window-system) (or (executable-find "pbpaste")
