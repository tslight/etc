...
#+begin_src emacs-lisp
  (use-package golden-ratio :ensure :demand :config (golden-ratio-mode))
#+end_src
...
Generally better to install from OS package manager to avoid extra
compilation step.

ie) https://www.freshports.org/misc/emacs-libvterm/ ;-)
...
#+begin_src emacs-lisp
  (use-package vterm :ensure
    ...
    :bind (:map vterm-mode-map
  	      ...
  	      ("C-l" . (lambda () (interactive) (vterm-send-key "l" nil nil t)))
...
#+begin_src emacs-lisp
  (when (and (not window-system) (executable-find "xclip"))
    (use-package xclip :ensure :hook (after-init . xclip-mode)))
#+end_src
