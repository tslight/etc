...
#+begin_src emacs-lisp
  ...
  (use-package copilot-chat :ensure
    :config
    (defun my/copilot-save ()
      "Save a Copilot chat buffer to ~/chats/%Y%m%d-%H%M%S.org"
      (interactive)
      (if (string-search "Copilot Chat" (buffer-name))
	  (let* ((chat-dir (expand-file-name "~/chats/"))
		 (timestamp (format-time-string "%Y%m%d-%H%M"))
		 (filename (concat chat-dir timestamp ".org")))
	    (unless (file-directory-p chat-dir)
	      (make-directory chat-dir t))
	    (write-region (point-min) (point-max) filename)
	    (message "Copilot chat saved to %s" filename))
	(message "This is not a *Copilot Chat buffer.")))
