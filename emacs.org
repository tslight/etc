...
#+begin_src emacs-lisp
  ...
  (keymap-global-set "C-c M-t p" 'transpose-paragraphs)
  (keymap-global-set "C-c M-t r" 'transpose-regions)
  (keymap-global-set "C-c M-t s" 'transpose-sexps)
...
#+begin_src emacs-lisp
(defun my/new-morning-page ()
  "Create Morning Page with Epoch time as name."
  (interactive)
  (find-file (format-time-string "~/Dropbox/mp/%s")))
#+end_src
...
#+begin_src emacs-lisp
  (defun apple-p ()
    (eq 0 (shell-command "grep -iq apple /var/log/messages")))

  (defun lenovo-p ()
    (eq 0 (shell-command "grep -iq lenovo /var/log/messages")))
