...
#+begin_src conf :tangle (if (retina-display-p) "~/.cwmrc" "no")
  gap 0 0 2 15
#+end_src

If less than 2000 pixels, 5 should be fine

#+begin_src conf :tangle (if (not (retina-display-p)) "~/.cwmrc" "no")
  gap 0 0 2 5
#+end_src
...
#+begin_src conf
  ...
  bind-key M-Escape window-cycle-ingroup
  bind-key MS-Escape window-rcycle-ingroup
  bind-key M-grave window-cycle-inclass
  bind-key MS-grave window-rcycle-inclass
...
#+begin_src conf :tangle (if (freebsd-p) "~/.cwmrc" "no")
...
#+begin_src conf :tangle (if (and (freebsd-p) (apple-p)) "~/.cwmrc" "no")
  bind-key 4-XF86MonBrightnessDown 'backlight - 5'
  bind-key 4-XF86MonBrightnessUp 'backlight + 5'
  bind-key 4-F1 'backlight - 5'
  bind-key 4-F2 'backlight + 5'
  bind-key 4-F10 'mixer vol.mute=toggle'
  bind-key 4-F11 'mixer vol=-5%'
  bind-key 4-F12 'mixer vol=+5%'
#+end_src

*** ThinkPad ONLY

#+begin_src conf :tangle (if (and (freebsd-p) (lenovo-p)) "~/.cwmrc" "no")
...
#+begin_src conf  :tangle (if (and (openbsd-p) (lenovo-p)) "~/.cwmrc" "no")
  bind-key 4-F1 'sndioctl output.mute=!'
  bind-key 4-F2 'sndioctl output.level=-0.05'
  bind-key 4-F3 'sndioctl output.level=+0.05'
  bind-key 4-F5 'xbacklight -dec 5'
  bind-key 4-F6 'xbacklight -inc 5'
#+end_src
