...
#+BEGIN_SRC common-lisp
  (defun get-cpu (output)
    (let* ((cpu-start (search "Package id 0:" output))
           (cpu-end (search "(high" output)))
      (subseq output cpu-start cpu-end)))

  (defun get-fan (output)
    (let* ((fan-start (search "Exhaust" output))
           (fan-end (search "(min" output)))
      (subseq output fan-start fan-end)))
