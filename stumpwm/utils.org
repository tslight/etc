...
#+BEGIN_SRC common-lisp
  (defun get-cpu (output)
    (let* ((start (search "Package id 0:" output))
           ...
           (int (parse-integer (remove-if #'alpha-char-p cpu) :junk-allowed t)))
      (cond ((< 60 int) (concat "^1*" cpu "^n"))
            ((< 50 int) (concat "^3*" cpu "^n"))
            (t cpu))))
  ...
  (defun get-fan (output)
    (let* ((start (search "Exhaust" output))
           ...
           (int (parse-integer (remove-if #'alpha-char-p fan) :junk-allowed t)))
      (cond ((< 3500 int) (concat "^1*" fan "^n"))
            ((< 2500 int) (concat "^3*" fan "^n"))
            (t fan))))
