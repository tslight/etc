...
#+BEGIN_SRC common-lisp
  (defcommand volume (action) ((:string "Enter a volume action: "))
    ...
    (let ((voltype (subseq action (1- (length action))))
          (output
           (run-shell-command
            (concat "amixer set Master " action "| awk '/%/ {print $5}' | uniq")
            t)))
      (cond ((string= voltype "+") (message "volume increased ~a" output))
            ((string= voltype "-") (message "volume decreased ~a" output))
            (t (message "volume toggle")))))
#+END_SRC
