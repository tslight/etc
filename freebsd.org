...
#+begin_src shell :tangle /sudo::/usr/local/bin/asmcfan :tangle-mode o755
  ...
  if [ "$SET_SPEED" -ne "$CUR_SPEED" ]; then
      logger -t asmcfan "Fan speed set to ${SET_SPEED}RPM (temp: ${TEMP}°C)"
      sysctl dev.asmc.0.fan.0.maxspeed="$SET_SPEED" dev.asmc.0.fan.1.maxspeed="$SET_SPEED"
  fi
...
#+begin_src shell :tangle /sudo::/usr/local/bin/fscksaver :tangle-mode o755
  ...
  # Check if battery is below 10% and not plugged in
  if [ "$battery_life" -lt 10 ] && [ "$ac_status" -eq 0 ]; then
      logger "Battery low ($battery_life%) and not on AC power. Shutting down..."
      shutdown -p now
  fi
...
#+BEGIN_SRC conf :tangle ~/.cwmrc
  ...
  bind-key 4-Return 'xterm -maximized -e emacsclient -nw -c -a ""'
