...
#+begin_src sh :tangle (if (eq 0 (shell-command "grep -iq apple /var/log/messages")) "/doas::/etc/crontab" "no")
...
#+name:
#+begin_src shell :tangle (if (eq 0 (shell-command "grep -iq apple /var/log/messages")) "/doas::/usr/local/bin/asmcfan" "no") :tangle-mode o755
  #!/bin/sh
...
#+begin_src conf :tangle (if (eq 0 (shell-command "grep -iq apple /var/log/messages")) "/doas::/usr/local/etc/X11/xorg.conf.d/50-libinput-kbd.conf" "no") 
  Section "InputClass"
...
#+begin_src conf :tangle (if (eq 0 (shell-command "grep -iq lenovo /var/log/messages")) "/doas::/usr/local/etc/X11/xorg.conf.d/50-libinput-kbd.conf" "no") 
  Section "InputClass"
	  Identifier "libinput keyboard catchall"
	  MatchIsKeyboard "on"
	  MatchDevicePath "/dev/input/event*"
