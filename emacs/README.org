...
2) Execute the elisp source code block below using ~C-c C-c~ with your cursor
   ...
   #+BEGIN_SRC emacs-lisp
     (org-babel-load-file (concat default-directory "/site-lisp/my-tangles.org"))
...
2) Run ~install.sh~, or the command below, in a terminal, from this directory:
   ...
   #+BEGIN_SRC sh
     emacs -Q --batch \
	   --eval '(org-babel-load-file "'$PWD'/site-lisp/my-tangles.org")' \
	   --eval '(my/tangle-directory "'$PWD'/")'
   #+END_SRC
