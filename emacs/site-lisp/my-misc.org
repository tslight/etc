...
#+begin_src emacs-lisp
  ...
  (defun my/gitlab-api-retriever (endpoint)
    "Retrieve resources from GitLab ENDPOINT."
    (interactive "sEnter an endpoint: ")
    (switch-to-buffer
     (let ((base-url "https://gitlab.com/api/v4/")
           (attr "?per_page=100&owned=true&membership=true")
           (url-request-method "GET")
           ;; https://stackoverflow.com/a/24188208
           (url-request-extra-headers `(("PRIVATE-TOKEN" . ,my/gitlab-api-key))))
       (url-retrieve-synchronously (concat base-url endpoint attr))))
    (delete-region (point-min) url-http-end-of-headers)
    (json-pretty-print-buffer)
    (json-read-from-string (buffer-string)))

  (defun my/gitlab-get-value (endpoint value)
    "Retrieve VALUE from GitLab ENDPOINT resources."
    (let ((response (with-temp-buffer
                      (let ((base-url "https://gitlab.com/api/v4/")
                            ...
                            ;; https://stackoverflow.com/a/24188208
                            (url-request-extra-headers `(("PRIVATE-TOKEN" . ,my/gitlab-api-key))))
                        (url-insert-file-contents (concat base-url endpoint attr))
                        (json-read)))))
      (mapcar (lambda (r) (cdr (assoc value r))) response)))

  (defun my/gitlab-project ()
    "Prompt for a GitLab project."
    (interactive)
    (let ((projects (my/gitlab-get-value "projects" 'name)))
