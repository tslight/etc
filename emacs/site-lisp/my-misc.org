...
#+begin_src emacs-lisp
  ...
  (defun my/gitlab-projects (directory)
    ...
    (let* ((projects (my/gitlab-get-values "projects" (list 'path_with_namespace
                                                            'ssh_url_to_repo)))
           (project (completing-read "Project to clone: "
                                     (mapcar  (lambda (prj)
                                                (cdr (assoc 'path_with_namespace prj)))
                                              projects)))
           (ssh_url (car (remove-if nil
                                    (mapcar
                                     (lambda (prj)
                                       (if (eq project (cdr (assoc 'path_with_namespace prj)))
                                           (cdr (assoc 'ssh_url_to_repo prj))))
                                     projects)))))
      (magit-clone-regular ssh_url directory nil)))

  (defun my/github-repos (directory)
    "Prompt for a Github repo, then clone that repo to DIRECTORY."
    (interactive "DDirectory to clone to: ")
    (let* ((repos (my/github-get-values "user/repos" (list 'full_name
                                                           'name
                                                           'ssh_url)))
           (repo (completing-read "Repo to clone: "
                                  (mapcar  (lambda (r)
                                             (cdr (assoc 'full_name r)))
                                           repos)))
           (sshurl (car (remove-if nil
                                   (mapcar
                                    (lambda (r)
                                      (if (eq repo (cdr (assoc 'full_name r)))
                                          (cdr (assoc 'ssh_url r))))
                                    repos))))
           (name (car (remove-if nil
                                 (mapcar
                                  (lambda (r)
                                    (if (eq repo (cdr (assoc 'full_name r)))
                                        (cdr (assoc 'name r))))
