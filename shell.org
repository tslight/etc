...
#+begin_src shell :tangle ~/.bashrc
  alias cp='cp -i'
  alias d='docker'
  command -v dysk &> /dev/null && alias df='dysk' || alias df='df -h'
  ...
  alias k='kubectl'
  alias lc='tokei'
  command -v lsd &> /dev/null && alias ls='lsd' || alias ls='ls --color=always'
  alias l='ls -a'
  alias ll='ls -la'
  alias m="make"
  alias mkdir='mkdir -p'
...
#+begin_src shell :tangle ~/.zshrc
  ...
  bindkey -e # ensure we use emacs bindings even though EDITOR=vi
#+end_src
** COMPLETION
#+begin_src shell :tangle ~/.zshrc
...
#+begin_src zsh :tangle ~/.zshrc
  prompt_vcs_setup() {
      ...
      local cwd="%B%F{yellow}:%F{magenta}%~%b%f"
