...
#+BEGIN_SRC common-lisp :tangle no
  ...
  (select-from-menu (current-screen)
                    (map 'list
                         (lambda (e) (list (pathname-name e)))
                         (directory "/usr/share/*/*.desktop")))

  (select-from-menu (current-screen)
                    (map 'list
                         (lambda (e) (ppcre:all-matches-as-strings
                                 "(?<=\\n(Name|Exec)\\=).*"
  ...
  (select-from-menu (current-screen)
                    (map 'list
                         ...
                         (apply #'append
                                (map 'list
                                     (lambda (e) (directory
                                             (concat e "/applications/*.desktop")))
                                     (split-seq (getenv "XDG_DATA_DIRS") ":")))))


  (select-from-menu (current-screen)
                    (map 'list
                         (lambda (e) (list (ppcre:scan-to-strings
                                       "(?<=\\nExec\\=).*(?=\\s\\%.*)"
                                       (uiop:read-file-string
                                        (pathname e)))))
                         (apply #'append
  ...
  (ppcre:all-matches-as-strings
   "(?<=\\n(Name|Exec)\\=).*"
   (uiop:read-file-string
    "/usr/share/applications/firefox-esr"))
