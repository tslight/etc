...
Clone my fork of the [[https://github.com/tslight/stumpwm-contrib/tree/merged][StumpWM Contrib]] repo to ~~/src/~, then switch to it's
~merged~ branch, in order to access much of the code that does the interesting
things present in my config.

#+BEGIN_SRC bash
mkdir ~/src
git clone https://github.com/tslight/stumpwm-contrib.git ~/src/stumpwm-contrib
cd ~/src/stumpwm-contrib
git checkout merged
#+END_SRC
