...
#+BEGIN_SRC common-lisp
  ...
  (set-module-dir
   (pathname-as-directory
    (concat (getenv "HOME") "/src/tspub/lisp/stumpwm-contrib")))
#+END_SRC
...
#+BEGIN_SRC common-lisp
  ...
  (run-shell-command "sct 5000")
  (run-shell-command "xset r rate 200 40 s 240 dpms 480 600 820")
  (run-shell-command "xrdb ~/.Xresources")
  ;; (run-shell-command "alttab -mk Control_L -kk Escape -vp pointer -d 1")
  ;; (run-shell-command "alttab -mk Alt_L -kk Escape -vp pointer")
  (run-shell-command "alttab -vp pointer -d 1")
  (run-shell-command "alttab -mk Super_L -vp pointer")
